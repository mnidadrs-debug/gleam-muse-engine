...
BEGIN
  ...
  SELECT coalesce(
           array_agg(distinct c.phone_number) FILTER (WHERE c.phone_number IS NOT NULL AND c.phone_number <> ''),
           '{}'
         )
  ...
  IF OLD.status IS DISTINCT FROM NEW.status AND NEW.status = 'ready' THEN
    BEGIN
      IF coalesce(array_length(v_cyclist_phones, 1), 0) = 0 THEN
        RAISE NOTICE 'No active available cyclists found for order % in neighborhood %', NEW.id, NEW.neighborhood_id;
      END IF;
...
DROP TRIGGER IF EXISTS orders_notify_status_webhooks ON public.orders;

CREATE TRIGGER orders_notify_status_webhooks
